...
function DPTobaccoDeliveryReport(props) {
  ...
  useEffect(() => {
    if (props.getDPTobaccoDeliveryReportData && props.getDPTobaccoDeliveryReportData.result) {
      setRowData(props.getDPTobaccoDeliveryReportData.result)
      setLoading(false)
    } else if (
      props.getDPTobaccoDeliveryReportData &&
      !props.getDPTobaccoDeliveryReportData.result
    ) {
      setRowData([])
      setLoading(false)
    }
  ...
  const columnDefs1 = [
    ...
    {
      headerName: t('AP_Bin_Date_label'),
      field: 'apBinDate',
      sortable: true,
      filter: true,
      tooltipField: 'apBinDate',
      cellRenderer: 'agGroupCellRenderer',
      lockPosition: true,
      width: 130,
      editable: false,
    ...
    {
      headerName: t('Sold_Date_label'),
      field: 'soldDate',
      sortable: true,
      filter: true,
      tooltipField: 'soldDate',
      cellRenderer: 'agGroupCellRenderer',
      lockPosition: true,
      width: 130,
      editable: false,
    ...
    if (rowData && rowData.length !== 0) {
      ...
      rowData.forEach((getDataItem, index1) => {
        newRowData.push({
          ...
          apBinDate: getDataItem.apBinDate,
          soldDate: getDataItem.soldDate,
          bagNumber: getDataItem.bagDetails[0].bagNo,
          truckNumber: getDataItem.bagDetails[0].truckNo,
        ...
        getDataItem.bagDetails.forEach((transactionItem, index2) => {
          if (index2 > 0) {
            newRowData.push({
              ...
              truckNumber: transactionItem.truckNo,
  ...
  const onBtExport = () => {
    var excelParams = {
      fileName: 'DP Tobacco Delivery Report.xlsx',
    }
